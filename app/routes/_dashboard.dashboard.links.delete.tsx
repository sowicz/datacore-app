...
export const action: ActionFunction = async ({ request }) => {
  ...
  const idToDel = formData.get("linkid") as string;
  const currentUserPassword = formData.get("password") as string;
  
  const user = await getUser(request);
  const id = user?.id || ""; 

  const result = await deleteLink({ id, currentUserPassword, idToDel });
  ...
  return (
    <div className="p-6 max-w-lg mx-auto mt-12">
      ...
      <p className="text-gray-600">
        If you confirm the deletion, the link will be removed permanently.
      </p>
